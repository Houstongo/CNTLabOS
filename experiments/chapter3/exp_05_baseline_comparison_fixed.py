...
class BaselineExperiment:
    ...
    def evaluate_bm25(self, queries: List[Dict]) -> Dict:
        ...
        for query in queries:
            ...
            for i, chunk in enumerate(retrieved_chunks[:5], 1):
                ...
                # 获取这个 chunk 对应的所有 MSFU（link_id）
                msfu_ids = self._get_chunk_msfu_ids(chunk_id)

                # 使用真实的 MSFU ID 格式：path_{msfu_id}
                for msfu_id in msfu_ids:
                    retrieved_msfus.add(f"path_{msfu_id}")
        ...
        for i, chunk in enumerate(chunks):
            ...
            for word in query.lower().split():
                if word in doc_freqs:
                    # IDF 计算：log(N / df)
                    idf = math.log(len(chunks) / doc_freqs[word] + 1) + 1
                    ...
                    # BM25 分数
                    numerator = tf * (k1 + 1)
                    denominator = tf + k1 * (1 - b + b * chunk_len / avgdl)
    ...
    def _get_all_chunks(self) -> List[Dict]:
        ...
        cursor = conn.execute("""
            ...
            FROM kb_chunks c
            INNER JOIN kb_links l ON c.id = l.chunk_id
            ORDER BY c.id
        """)
    ...
    def _get_chunk_msfu_ids(self, chunk_id: int) -> List[int]:
        """获取 chunk 对应的 MSFU ID 列表（kb_links.id）"""
        import sqlite3
        conn = sqlite3.connect(self.kb_path)
        conn.row_factory = sqlite3.Row
        ...
        msfu_ids = [row['id'] for row in cursor.fetchall()]
        conn.close()
        return msfu_ids
