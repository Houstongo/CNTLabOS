...
INPUT_CSV = PROJECT_ROOT / "reports" / "50000x_feature_extract_20260401" / "no48_50000x_modeling_table.csv"
OUTPUT_DIR = PROJECT_ROOT / "reports" / "50000x_feature_extract_20260401" / "no48_50000x_modeling_baselines"

TARGETS = [
    "mean_curvature_um_inv",
    "waviness_ratio",
    "tortuosity",
    "alignment",
    "density",
...
def build_model_specs() -> list[dict]:
    return [
        {
            "name": "ridge_numeric_raw",
            "num_cols": ["fe_power_w", "fe_thickness_nm"],
            "cat_cols": [],
            "estimator": Ridge(alpha=1.0),
        },
        {
            "name": "ridge_power_cat",
            "num_cols": ["fe_thickness_nm"],
            "cat_cols": ["fe_power_w"],
            "estimator": Ridge(alpha=1.0),
        },
        {
        ...
        {
            "name": "rf_raw_plus_index",
            "num_cols": ["fe_power_w", "fe_thickness_nm", "fe_deposition_index"],
            "cat_cols": [],
...
def evaluate_models(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, dict[str, np.ndarray | dict]]]:
    ...
    groups = df["field_group"].astype(str)
    ...
    for target in TARGETS:
        ...
        target_groups = target_df["field_group"].astype(str)
        best_row = None
        best_pred = None
        ...
        for spec in model_specs:
            ...
            y_pred = cross_val_predict(pipeline, x, y, cv=cv, groups=target_groups, n_jobs=None)
    ...
    result_df = pd.DataFrame(rows).sort_values(["target", "r2"], ascending=[True, False]).reset_index(drop=True)
...
def save_prediction_scatter(best_predictions: dict[str, dict[str, np.ndarray | dict]], output_dir: Path) -> None:
    ...
    for ax, target in zip(axes.ravel(), TARGETS):
        ...
        ax.set_xlabel("True")
        ax.set_ylabel("Pred")
    fig.suptitle("No48 + 50000x Baseline Modeling", fontsize=15)
    fig.savefig(output_dir / "no48_50000x_prediction_scatter.png", dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def save_outputs(df: pd.DataFrame, results: pd.DataFrame, best_predictions: dict[str, dict[str, np.ndarray | dict]], output_dir: Path) -> None:
    ...
    summary = {
        ...
        "group_count": int(df["field_group"].nunique()),
        "recommended_main_inputs": ["fe_power_w", "fe_thickness_nm", "fe_deposition_index"],
        "recommended_main_targets": ["mean_curvature_um_inv", "waviness_ratio", "tortuosity", "alignment"],
    ...
    md_lines = ["# No48 + 50000x Baseline Modeling", ""]
    md_lines.append(f"- rows: `{len(df)}`")
    md_lines.append(f"- grouped fields: `{df['field_group'].nunique()}`")
    md_lines.append("- CV: `GroupKFold(5)` by `field_group`")
    md_lines.append("")
