...
OUTPUT_DIR = BASE_DIR / "target_specialized_kb_experiment"
...
def build_target_specs(kb_cols: Iterable[str]) -> dict[str, list[dict]]:
    ...
    return {
        ...
        "waviness_ratio": [
            ...
            {
                "name": "ridge_waviness_kb",
                "num_cols": [
                    "fe_thickness", "anneal_time", "fe_deposition_index",
                    "anneal_time_x_thickness", "deposition_time_index", "time_sq", *kb_cols,
                ],
                "cat_cols": ["power_bin", "anneal_power_combo"],
...
def main() -> None:
    ...
    for target in TARGETS:
        ...
        target_df = df.merge(priors, on="image_id", how="left")
        results_frames.append(evaluate_target(target_df, target, kb_cols))
        prior_meta[target] = {"query": build_query_for_target(target), "kb_feature_columns": kb_cols}
