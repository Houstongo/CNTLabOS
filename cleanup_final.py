...
# We want to remove all copies of the ML logic and replace with ONE clean block.
# Definitions of what to look for
start_marker = "        function onMlVizOptionChange() {"
end_marker = "            mlVizState.selectedPoint = null;"
insert_marker = "// --- ML CORE LOGIC START ---"
        ...
        function renderXrMlCharts(rows) {
            ...
            if (panel) {
                ...
                panel.innerHTML = `
                    ...
                    <div class="flex items-center justify-between"><span class="text-slate-500">均值</span><span class="font-bold text-slate-700">${fmtMl(yAvg, 3)}</span></div>
                `;
            }
...
while i < len(lines):
    ...
    if skip:
        if end_marker in line:
            # Check if this is truly the end (next few lines should be next function)
            skip = False
        i += 1
        continue
...
print(f"File cleaned successfully. Found markers: {found_any}")
